# Dietary Nitrate and Physical Performance

- **Autori**: Andrew M. Jones, Christopher Thompson, Lee J. Wylie, Anni Vanhatalo
- **Anno**: 2018 · **Rivista**: Annual Review of Nutrition
- **DOI**: https://doi.org/10.1146/annurev-nutr-082117-051622 · **PMID**: 30130468
- **Provenienza banche dati**: openalex
- **Scoperto via**: query
- **Open access**: [bronze](https://www.annualreviews.org/doi/pdf/10.1146/annurev-nutr-082117-051622)

**Verifica metadati**: ✔ verificato

  - ⚠ [PubMed] Primo autore discordante: 'andrew' vs 'jones'

## Qualità metodologica
- Tipo di studio: **narrative review**
- Qualità: **moderate** · Confidenza: **moderate** · Trasferibilità ciclisti competitivi: **moderate**
  - sample: moderate
  - control: low
  - randomization: low
  - duration: moderate
  - measures: moderate
  - transferability: moderate
  - _Review fisiologica ampia, ma non sistematica._
  - _Criteri di ricerca, selezione e valutazione del rischio di bias non riportati._
  - _Include prestazione di endurance, potenza e sprint, ma popolazioni e protocolli sono eterogenei._
  - _Le conclusioni non sono specifiche per ciclisti competitivi._

## Dati estratti (fonte: abstract, metodo: llm)
- **Disegno dello studio** (`abstract`): Review
- **Partecipanti**: _non disponibile_
- **Livello di allenamento**: _non disponibile_
- **Sesso**: _non disponibile_
- **Età**: _non disponibile_
- **Numerosità campione**: _non disponibile_
- **Protocollo** (`abstract`): Review of the physiological mechanisms, circumstances associated with ergogenic effects, and effects of dietary nitrate supplementation on physical performance in a range of human populations.
- **Comparatore**: _non disponibile_
- **Durata**: _non disponibile_
- **Outcome** (`abstract`): Muscle efficiency, O2 cost of submaximal exercise, endurance exercise performance, skeletal muscle contractile function, muscle power, and sprint exercise performance.
- **Risultati** (`abstract`): Recent studies indicate that dietary nitrate supplementation, most commonly administered as beetroot juice, can reduce the O2 cost of submaximal exercise and improve endurance exercise performance, and can enhance skeletal muscle contractile function, muscle power, and sprint exercise performance.
- **Effect size**: _non disponibile_
- **Limiti**: _non disponibile_
- **Conflitti di interesse**: _non disponibile_

## Abstract
Nitric oxide (NO) plays a plethora of important roles in the human body. Insufficient production of NO (for example, during older age and in various disease conditions) can adversely impact health and physical performance. In addition to its endogenous production through the oxidation of l-arginine, NO can be formed nonenzymatically via the reduction of nitrate and nitrite, and the storage of these anions can be augmented by the consumption of nitrate-rich foodstuffs such as green leafy vegetables. Recent studies indicate that dietary nitrate supplementation, administered most commonly in the form of beetroot juice, can ( a) improve muscle efficiency by reducing the O 2 cost of submaximal exercise and thereby improve endurance exercise performance and ( b) enhance skeletal muscle contractile function and thereby improve muscle power and sprint exercise performance. This review describes the physiological mechanisms potentially responsible for these effects, outlines the circumstances in which ergogenic effects are most likely to be evident, and discusses the effects of dietary nitrate supplementation on physical performance in a range of human populations.
