# Carbohydrate Availability and Physical Performance: Physiological Overview and Practical Recommendations

- **Autori**: Fernando Mata Ordóñez, Pedro L. Valenzuela, Jaume Gimenez, Carles Tur, Diogo Ferreria, Raúl Domínguez, Antonio Jesús Sánchez-Oliver, José Miguel Martínez Sanz
- **Anno**: 2019 · **Rivista**: Nutrients
- **DOI**: https://doi.org/10.3390/nu11051084 · **PMID**: 31100798
- **Provenienza banche dati**: openalex
- **Scoperto via**: query
- **Open access**: [gold](https://www.mdpi.com/2072-6643/11/5/1084/pdf?version=1557987590)

**Verifica metadati**: ✔ verificato

  - ⚠ [PubMed] Primo autore discordante: 'fernando' vs 'mata'

## Qualità metodologica
- Tipo di studio: **revisione narrativa con raccomandazioni pratiche**
- Qualità: **moderate** · Confidenza: **moderate** · Trasferibilità ciclisti competitivi: **high**
  - sample: moderate
  - control: low
  - randomization: low
  - duration: high
  - measures: moderate
  - transferability: high
  - _Integra fisiologia, prestazione e periodizzazione della disponibilità di carboidrati._
  - _La materia è direttamente pertinente alle richieste energetiche e agli adattamenti del ciclismo competitivo._
  - _Non sono descritti ricerca sistematica, selezione degli studi, rischio di bias o metodi di sintesi._
  - _Le raccomandazioni sono plausibili ma generali e richiedono personalizzazione per durata, intensità, obiettivo della sessione e tolleranza gastrointestinale._

## Dati estratti (fonte: abstract, metodo: llm)
- **Disegno dello studio** (`abstract`): Narrative review
- **Partecipanti** (`abstract`): Athletes
- **Livello di allenamento**: _non disponibile_
- **Sesso**: _non disponibile_
- **Età**: _non disponibile_
- **Numerosità campione**: _non disponibile_
- **Protocollo** (`abstract`): Holistic review of how carbohydrate availability affects physical performance and how training and nutrition can be combined
- **Comparatore** (`abstract`): Low versus high carbohydrate availability
- **Durata**: _non disponibile_
- **Outcome** (`abstract`): Exercise performance, exercise-induced adaptations, mitochondrial biogenesis, lipolysis, and resistance-training adaptations
- **Risultati** (`abstract`): Glycogen depletion limits performance when high exercise intensities are required, making high carbohydrate availability before and during such exercise important. High carbohydrate availability is positively related to resistance-training adaptations, whereas low availability may promote endurance adaptations such as mitochondrial biogenesis and enhanced lipolysis. No universal carbohydrate intake recommendation can be given; nutrition should be periodized according to training loads and objectives.
- **Effect size**: _non disponibile_
- **Limiti** (`abstract`): No universal recommendations regarding carbohydrate intake can be given.
- **Conflitti di interesse**: _non disponibile_

## Abstract
Strong evidence during the last few decades has highlighted the importance of nutrition for sport performance, the role of carbohydrates (CHO) being of special interest. Glycogen is currently not only considered an energy substrate but also a regulator of the signaling pathways that regulate exercise-induced adaptations. Thus, low or high CHO availabilities can result in both beneficial or negative results depending on the purpose. On the one hand, the depletion of glycogen levels is a limiting factor of performance during sessions in which high exercise intensities are required; therefore ensuring a high CHO availability before and during exercise is of major importance. A high CHO availability has also been positively related to the exercise-induced adaptations to resistance training. By contrast, a low CHO availability seems to promote endurance-exercise-induced adaptations such as mitochondrial biogenesis and enhanced lipolysis. In the present narrative review, we aim to provide a holistic overview of how CHO availability impacts physical performance as well as to provide practical recommendations on how training and nutrition might be combined to maximize performance. Attending to the existing evidence, no universal recommendations regarding CHO intake can be given to athletes as nutrition should be periodized according to training loads and objectives.
